...
This is a Go package named `eventkit` that provides tools for testing and interacting with various protocols and event brokers. The package includes some command-line tools  that allows users to send and receive messages using protocols like CoAP, MQTT, NATS, and HTTP. The tools are designed to facilitate testing and debugging of event-driven systems by providing functionalities such as sending test payloads, subscribing to topics, and measuring performance.
